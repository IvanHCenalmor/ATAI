            ...
            while (aimed("false") & bucle(X) & (X < Length)) {
                ...
                if (Type > 1000) {
                    ...
                    if (Team == 100) {  // Only if I'm AXIS
                        ...
                        //First of all it is told to the team that an enemy has been shown

                        .nth(6, Object, NewDestination);
                        
                        .my_team("AXIS", E1);
                        .concat("enemy(",NewDestination,")", Content);
                        .send_msg_with_conversation_id(E1, tell, Content, "ENEMY");
                        ?debug(Mode); if (Mode<=2) {.println("Enemy detected");}

						.nth(3, Object, Angle);
						+bucle2(0);
						+friendly(0);
						?debug(Mode); if (Mode<=2) {.println("There is an enemy: ", Object);}
						while(friendly(F) & F == 0 & bucle2(J) & (J < Length)){
							...
							if(not(Type2 > 1000)){
								.nth(1,Object2,Team2);
								if (Team2 == 200){
									?debug(Mode); if (Mode<=2) {.println("There is also an allied: ", Object2);}
									.nth(3, Object2, Angle2);
									if(((Angle - 5) < Angle2) & (Angle2 <= (Angle + 5))){
										?debug(Mode); if (Mode<=2) {.println("It is in the angle");}
										-+friendly(1);
									}
								}
							}
						...
						if(Friend == 0){
							?debug(Mode); if (Mode<=2) {.println("Shoot!");}
							+aimed_agent(Object);
							-+aimed("true");
						}else{
							?debug(Mode); if (Mode<=2) {.println("Do not shoot!");}
						}
...
+enemy(Destination)[source(M)]
    <-  ?debug(Mode); if (Mode<=2) {.println("Going for the enemy");}
